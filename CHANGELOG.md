# Changelog

This changelog records public-safe repository improvements.

## Unreleased

### Added

- Public-safe repository README.
- Public proof manifest.
- Safe public scope document.
- Redaction checklist.
- Seven-day GitHub balance plan.
- Lightweight CI boundary check.
- First public-safe learning note.
- Public-safe note template.
- Review before proof checkpoint.
- Public proof discipline note.
- Repeatable proof reflection note.
- Public review guide.
- Start here guide.
- Weekly Note 001: Evidence Before Decision.
- Public Surface Review 001.
- Public reviews index.
- Weekly Note 002: Review Before Claim.
- Daily proof pulse for 2026-07-08.
- Daily proof pulse for 2026-07-11.

### Changed

- Repackaged the README as a clearer Maximelion founder proof surface.
- Surfaced the latest daily proof pulse in the README proof trail.
- Updated the latest daily proof pulse route to 2026-07-11.

## Changelog Rules

Future entries should focus on public repository discipline:

- notes added
- templates added
- checklists improved
- CI or documentation checks improved
- issue and pull request rhythm improved
- public-safe documentation structure improved

Future entries should stay high-level and public-safe.
